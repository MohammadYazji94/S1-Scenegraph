...
function(exports, scenegraph, animation) {
	...
	function init() {
		...
		var schultergelenk = scenegraph.createNodeWithModel("Schultergelenk", "sphere", {scale:120, recursionDepth:4});
		schultergelenk.translate([-500,200,0]);
		schultergelenk.rotateTo([0, 6.2, 0]);

		var oberarm = scenegraph.createNodeWithModel("Oberarm", "cube", {scale:100}, schultergelenk);
		oberarm.scale([1, 0, 0]);
		oberarm.rotateTo([1.6, -3.87, 0]);
		oberarm.translate([210,-220, 0]);

		var ellenbogengelenk = scenegraph.createNodeWithModel("Ellenbogengelenk", "sphere", {scale:100, recursionDepth:4}, schultergelenk);
		ellenbogengelenk.translate([420, -430, 0])

		var unterarm = scenegraph.createNodeWithModel("Unterarm", "cube", {scale:100}, ellenbogengelenk);
		unterarm.scale([0.8, 0, 0]);
		unterarm.translate([280, 0, 0]);
		unterarm.rotateTo([0, 0, 0]);

		var hand = scenegraph.createNodeWithModel("Hand", "sphere", {scale:80, recursionDepth:4}, ellenbogengelenk);
